...
function main() {
  ...
  const scriptDir = path.resolve(__dirname, ".."); // project root (since script is in js/)
  const normalizedRoot = path.resolve(scriptDir, root);
  ...
  const imageRefRegex =
    /[A-Za-z0-9_\-./%]+\.(?:png|jpe?g|gif|svg|webp|avif)/gi;
  ...
  const unusedImages = Array.from(allImageSet).filter(
    (p) => !referencedAbsPaths.has(p)
  );
  ...
  if (shouldDelete) {
    ...
    console.log(
      `Moved ${unusedImages.length} unused images to: ${path.join(
        normalizedRoot,
        "_unused_images"
      )}`
    );
